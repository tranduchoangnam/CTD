...
Type* compileLValue(void) {
  ...
  switch (var->kind) {
    case OBJ_VARIABLE:
      ...
      varType = compileIndexes(varType); // parse array indexes if any
      break;
    case OBJ_PARAMETER:
      varType = var->paramAttrs->type;
      break;
    case OBJ_FUNCTION:
      // must be the current function to be a valid lvalue
...
// Update to allow multiple assignments
void compileAssignSt(void) {
  ...
  exprs[exCount++] = compileExpression();
  while (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    exprs[exCount++] = compileExpression();
  }
  ...
  // Type check each pair
  for (int i = 0; i < lwCount; i++) {
    checkTypeEquality(lvalues[i], exprs[i]);
  }
}
...
void compileCallSt(void) {
  ...
  // Call compileArguments with new signature that tracks argument count
  Type** argTypes = compileArguments(proc->procAttrs->paramList, &argCount);
  if (argTypes) {
    free(argTypes); // Free allocated memory
  }
}
...
Type** compileArguments(ObjectNode* paramList, int* argCount) {
  Type** argTypes = malloc(sizeof(Type*) * 64);
  if (!argTypes) {
  ...
  switch (lookAhead->tokenType) {
    case SB_LPAR:
      ...
      if (lookAhead->tokenType != SB_RPAR) {
        argTypes[count++] = compileExpression();
        while (lookAhead->tokenType == SB_COMMA) {
          eat(SB_COMMA);
          argTypes[count++] = compileExpression();
        }
...
Type* compileExpression(void) {
  ...
  //   while (lookAhead->tokenType == SB_COMMA) {
  //     eat(SB_COMMA);
  //     Type* nextType = compileExpression2();
...
Type* compileExpression3(void) {
  ...
  default:
    ...
    return NULL;  // Add return after error
  }
}
    ...
    case TK_IDENT:
      ...
      switch (obj->kind) {
        ...
        case OBJ_FUNCTION:
          ...
          {
            int argCount = 0;
            Type** argTypes = compileArguments(obj->funcAttrs->paramList, &argCount);
            free(argTypes);
          }
          break;
          break;
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    checkIntType(idxType); // index must be int
